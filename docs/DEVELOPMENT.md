...
- Inspect `/api/monitor/status` for submitted/completed/error counts, latency, reconnect state, and redacted errors. During an in-flight model call, `last_capture_at` and `submitted_count` must remain unchanged until that logical analysis cycle finishes.
