...
Temporal filtering defaults to a five-result window, three-result confirmation, and three consecutive lower-severity results to clear. Stable boxes use same-category IoU `0.2` and EMA alpha `0.35`. These settings operate only on validated structures; see [Temporal stabilization](STABILIZATION.md) before changing them. A provider failure must never be inserted as a normal vote.
...
`RTSP_STALL_TIMEOUT_SECONDS` defaults to 30 seconds and must be a finite positive number. It configures the continuous FFmpeg socket I/O timeout and the watchdog between complete JPEG frames. The configured model minimum frame interval does not alter this timeout because preview decoding follows the RTSP source rather than model cadence. A reconnect test must prove watchdog expiry terminates the child process before the backoff state is published.

The production image builds the exact FFmpeg release pinned by `FFMPEG_VERSION` and `FFMPEG_SHA256` in `Dockerfile` from the official `ffmpeg.org` source archive. Do not replace it with an unversioned distribution package. The image build must exercise an RTSP open attempt using the RTSP-native `timeout`/transport options and execute a finite synthetic run of the continuous MJPEG `image2pipe` path without timed sampling or resizing. Do not use the generic `rw_timeout` for RTSP: FFmpeg 8.1.2 lists it in global protocol help but rejects it when the RTSP demuxer opens the input. An argument-list unit test or help-text grep alone does not prove that the runtime binary accepts an option in the target demuxer context.
...
- Inspect `/api/monitor/status` for submitted/completed/error counts, latency, reconnect state, and redacted errors. During an in-flight model call, `last_capture_at` should continue advancing with the preview while `submitted_count` remains unchanged until the next logical analysis cycle begins.
- RTSP failures should begin with an actionable category and may end with bounded `技术信息`. If the UI renders `[object Object]`, inspect the response body shape and route it through the shared frontend API-detail formatter instead of string-coercing `detail`.
- Inspect `/api/alarm` for the current raw/stable risk, vote support, stable signals/boxes, and current-session timeline. Compare the stable overlay with the raw overlay before changing association or hysteresis thresholds.
